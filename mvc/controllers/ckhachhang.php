...
class ckhachhang extends controller
{
    ...
    public function thuchiendangky()
    {
        ...
        if ($row > 0) {
            ...
            echo "<script>location.replace('/Project3/ctrangchu/')</script>";
        } else {
            echo '<script>alert("Đăng ký thất bại!")</script>';
            echo "<script>location.replace('/Project3/ckhachhang/dangky/')</script>";
        }
    }
    // thuc hien dang nhap
    public function thuchiendangnhap()
    {
        ...
        if (count($admin) > 0) {
            ...
            $this->href("/Project3/cquanlimt/quanli/");
        } else {
            ...
            if (count($khachhang) > 0) {
                if ($khachhang[0]['kichhoat'] == 1) {
                    ...
                    if (isset($_SESSION['mamt'])) {
                        echo $this->href("/Project3/cmaytinh/getsanphambymamt/" . $_SESSION['mamt']);
                    } else {
                        echo $this->href('/Project3/ctrangchu/');
                    }
                } else {
    ...
    public function themkh()
    {
        ...
        if ($row > 0) {
            ...
            echo "<script>location.replace('/Project3/ckhachhang/danhsachkhachhang/')</script>";
        } else {
            echo '<script>alert("Thêm khách hàng không thành công!")</script>';
            // echo "<script>location.replace('/Project3/cquanlimt/themmt/')</script>";
        }
    }
    public function capnhattaikhoan()
    {
        ...
        if ($namehinhanh == null) {
            ...
            echo $this->href("http://localhost/Project3/ckhachhang/thuchiencapnhattk/");
        } else {
            $row = $this->khachhang->updatekh($makh, $hoten, $matkhau, $sdt, $namehinhanh);
            if ($row > 0) {
                echo '<script>alert("Cập nhật thành công!")</script>';
                echo "<script>location.replace('/Project3/ckhachhang/khcapnhatthongtin/')</script>";
            } else {
    ...
    public function duyetkhachhang($makh)
    {
        ...
        $this->href("/Project3/ckhachhang/danhsachkhachhang/");
    }
    //
